...
for i in range(len(applications)):
    ...
    for feature_id in feature_ids:
        ...
        # store each feature
        df_applications = save_working_item_into_data_frame(feature_id, df_applications)

        user_stories = show_children(feature_id)
    
        # go through childs of features (user_stories
        for user_story in user_stories:
            # store each user story
            df_applications = save_working_item_into_data_frame(user_story, df_applications)
...
# saves to csv file
df_applications.to_csv('file_name_140x.csv')
